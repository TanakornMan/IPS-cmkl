...
\begin{frame}[plain,noframenumbering]
  % White bar with logos at top
  \begin{tikzpicture}[remember picture,overlay]
    % White box that starts from left and ends before the right edge
    \fill[white] (current page.north west) rectangle ([xshift=-1cm,yshift=-1.8cm]current page.north east);
    % Yellow border - only bottom and right edges (not touching page borders)
    \draw[line width=0.4pt,color=mLightBrown] ([yshift=-1.8cm]current page.north west) -- ([xshift=-1cm,yshift=-1.8cm]current page.north east);
    \draw[line width=0.4pt,color=mLightBrown] ([xshift=-1cm]current page.north east) -- ([xshift=-1cm,yshift=-1.8cm]current page.north east);
    \node[yshift=-0.9cm] at (current page.north) {
      \includegraphics[height=1.2cm]{logo/awist.png}
      \hspace{1.5cm}
      \includegraphics[height=1.2cm]{logo/muic_logo.jpg}
      \hspace{1cm}
      \includegraphics[height=1.2cm]{logo/cmkl_logo.png}
    };
  \end{tikzpicture}
  \vspace{3cm}
  \titlepage
